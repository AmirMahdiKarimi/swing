...
# اضافه کردن Scripts به sys.path
...
def main():
    ...
    ap.add_argument('--output_dir', default=os.path.join('Results', 'XGBoost'))
    ...
    ap.add_argument('--mhc_grouping', choices=['none', 'locus', 'two_digit'], default='none', help='کاهش کاردینالیتی MHC با گروه‌بندی')
    ...
    # وزن کلاس مثبت بعد از (یا بدون) اورسمپلینگ
    pos = (y_train == 1).sum()
    neg = (y_train == 0).sum()
